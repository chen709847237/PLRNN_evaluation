...
# KL divergence with Monte Carlo approximation
...
def calc_kl_from_data(mu_gen, data_true):
    ...
    scaling = 1.
    cov_inf = tc.ones(data_true.shape[-1]).repeat(time_steps, 1)*scaling    # 真实数据 方差 全1矩阵 [9640, 64]
    cov_gen = tc.ones(data_true.shape[-1]).repeat(time_steps, 1)*scaling    # 预测数据 方差 全1矩阵 [9640, 64]
